...
%% Checking the trained data 
x_train_pre = zeros(4,15000);
y_train = zeros(4,15000);
for neuron_no = 1:4
    pred = conv(Stimulus(1:15000),h_t(neuron_no,:));
    x_train_pre(neuron_no,:) = pred(1:15000);
end
for neuron_no = 1:4
    for i = 1:15000
        x_train_pre(neuron_no,i) = (fit_const{neuron_no,1}.a)/(1+exp(-fit_const{neuron_no,1}.b*(x_train_pre(neuron_no,i)-fit_const{neuron_no,1}.c)));
    end
end
for neuron_no = 1:4
        y_train(neuron_no,:) = PSTH(neuron_no,1:15000);
...
x_mean_train_pre = zeros(4,1500);
y_mean_train_pre = zeros(4,1500);
bin_x_train = 10;
bn_sz_train = 1:bin_x_train:15000;
for neuron_no = 1:4
    for j = 1:ceil(15000/bin_x_train) 
        x_mean_train_pre(neuron_no,j) = mean(x_train_pre(neuron_no,bn_sz_train(j):bn_sz_train(j)+(bin_x_train-1)));
        y_mean_train_pre(neuron_no,j) = mean(y_train(neuron_no,bn_sz_train(j):bn_sz_train(j)+(bin_x_train-1)));
    end
    figure(13);
    subplot(2,2,neuron_no);
    plot(bn_sz_train,y_mean_train_pre(neuron_no,:));
    hold on;
    plot(bn_sz_train,x_mean_train_pre(neuron_no,:));
    ylabel('λ(t)');
    xlabel('Time (ms)');
...
for neuron_no = 1:4
    r_value = corrcoef(y_train(neuron_no,:),x_train_pre(neuron_no,:));
...
for neuron_no = 2
    ...
    while (prev_r_sqr - next_r_sqr) < 0.014
        ...
        index = 0;
        pred_pruned = conv(Stimulus(1:15000),h_t(neuron_no,:));
        x_train_pruned(neuron_no-1,:) = pred_pruned(1:15000);
        for i = 1:15000
            x_train_pruned(neuron_no-1,i) = (fit_const{neuron_no,1}.a)/(1+exp(-fit_const{neuron_no,1}.b*(x_train_pruned(neuron_no-1,i)-fit_const{neuron_no,1}.c)));
        end
        r_value = corrcoef(y_train(neuron_no,:),x_train_pruned(neuron_no-1,:));
...
for neuron_no = 3
    ...
    index = 0;
    count = 0;
    while (prev_r_sqr - next_r_sqr) < 0.01
        prev_r_sqr = next_r_sqr;
        min_va = max(h_t(neuron_no,:));
        for i = 1:100
            if abs(h_t(neuron_no,i))<min_va && abs(h_t(neuron_no,i)) ~= 0
...
%
mean_err_abs_2 = mean(abs(y_test(2,:)-x_test(2,:)))/mean(abs(y_test(2,:)));
mean_err_abs_3 = mean(abs(y_test(3,:)-x_test(3,:)))/mean(abs(y_mean_test(2,:)));
display(mean_err_abs_2);
display(mean_err_abs_3);
